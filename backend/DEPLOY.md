...
3. DIY 目前預留 4 個新增位置；填寫「可新增空白項目」的名稱、價格、時長、成團人數與圖片，再勾選「顯示此項目」。取消勾選即可下架，內容仍會保留。
4. FAQ 的問題列為綠色、答案列為黃色；表格型答案也會緊接在所屬問題下方。
5. 完成後勾選第 5 列「我已確認，儲存全部變更」。
6. 確認狀態顯示「已儲存 N 項到 Sheet ✓」，並在體驗頁重新整理驗證。
7. 測試後把價格改回正確值，再儲存一次。
