...
(function() {
    ...
    if (prodGrid && window.DATA.products) {
        prodGrid.innerHTML = window.DATA.products.map(function(p) {
            return '<div class="text-center"><div class="w-full aspect-square rounded-full overflow-hidden border-8 border-[#fffcf5] shadow-lg mb-10 mx-auto max-w-[320px]"><img src="' + p.image + '" class="!w-full !h-full !object-cover !block" alt="' + p.name + '｜大花農場玫瑰伴手禮"></div><h3 class="text-3xl font-serif text-primary font-bold mb-4">' + p.name + '</h3><p class="text-gray-500 text-2xl line-clamp-2">' + p.desc + '</p></div>';
        }).join('');
    }
    ...
    if (diyGrid && window.DATA.diy) {
        diyGrid.innerHTML = window.DATA.diy.map(function(d) {
            return '<div class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all hover:-translate-y-2 cursor-pointer group" onclick="window.openModal(\'' + d.modal + '\')">'
                + '<div class="h-96 relative overflow-hidden"><img src="' + d.image + '" class="!absolute !inset-0 !w-full !h-full !object-cover !block transition-transform duration-500 group-hover:scale-110" alt="' + d.name + '｜大花農場手作體驗">'
